...
public class forgotPassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int strConvertIntoInt = Integer.parseInt(id);
            ...
            try {
                ...
                statement.setInt(1, strConvertIntoInt);
                ResultSet rs = statement.executeQuery();

                if (rs.next() == true) {
                    if (passwordOne.equals(passwordTwo)) {
                        PreparedStatement statement1 = connection.prepareStatement("UPDATE EMPLOYEE SET EMP_PASSWORD = ? WHERE EMP_ID = ?");
                        statement1.setString(1, passwordOne);
                        statement1.setInt(2, strConvertIntoInt);
                        statement1.executeUpdate();
                        response.sendRedirect("s/sucess.jsp");
                    } else {
                        out.print("<h1 style=' position: absolute; text-align: center; top:40%; left:34%;'>What happens if your password  is not verified ?...  </h1>");
